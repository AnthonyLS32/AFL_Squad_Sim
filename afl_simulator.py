...
player_pool = [
    ...
    {"name": "Jack Steele", "position": "Midfield", "ovr": 87, "photo_url": "https://via.placeholder.com/100x130.png?text=Jack+Steele"},import streamlit as st
...
player_pool = [
    ...
    # 2 Bench Wildcards
    {"name": "Wildcard 1", "position": "Forward", "ovr": 75, "photo_url": "https://via.placeholder.com/100x130?text=Wildcard+1"},
    {"name": "Wildcard 2", "position": "Midfield", "ovr": 74, "photo_url": "https://via.placeholder.com/100x130?text=Wildcard+2"}
]
...
# --- UI ---
st.title("AFL Career Squad Selector")
st.write(f"Selected: {len(st.session_state.selected_team)}/22")

for player in st.session_state.squad:
    is_sel = player in st.session_state.selected_team
    card_class = "player-card selected" if is_sel else "player-card"
    ...
    c1, c2 = st.columns(2)
    with c1:
        if not is_sel and st.button(f"Select {player['name']}", key=f"sel_{player['name']}"):
            st.session_state.click_action = "select"
            st.session_state.click_player = player
    with c2:
        if is_sel and st.button(f"Deselect {player['name']}", key=f"des_{player['name']}"):
...
if st.session_state.click_action == "select":
    ...
    limit = position_limits.get(p['position'], 0)
    if count_pos(p['position']) < limit:
        st.session_state.selected_team.append(p)
    elif p['position'] in ["Forward", "Midfield", "Defender", "Ruck"]:
        st.warning(f"Max {p['position']}s reached.")
    else:
        st.warning("Cannot add more Bench players.")
    st.session_state.click_action = None
    st.session_state.click_player = None
    st.experimental_rerun()

elif st.session_state.click_action == "deselect":
    st.session_state.selected_team.remove(st.session_state.click_player)
    st.session_state.click_action = None
    st.session_state.click_player = None
    st.experimental_rerun()
...
if st.button("Save Team"):
    if len(st.session_state.selected_team) < 22:
        st.warning("Must have 22 players.")
    else:
