...
if os.path.exists('save_squad.json'):
    with open('save_squad.json') as f:
...
if 'squad' not in st.session_state:
    ...
    if saved_squad:
        ...
        # Pick initial squad: realistic AFL match day squad
        squad = pick('Forward', 6) + pick('Midfield', 6) + pick('Ruck', 2) + pick('Defender', 6) + pick(None, 2)
...
# --- Helper: full stats line with photo ----------------------------------------------
def stats(p):
...
# --- SELECTED TEAM TAB --------------------------------------------------------------
    ...
    for pos, count in slots:
        ...
        for i in range(count):
            ...
            if not names:
                st.warning(f"No more available players for {pos}.")
                continue
    ...
    if st.button("Save Selected Team"):
        if len(selections) == sum(count for _, count in slots):
            ...
            # Save squad on team save
            with open('save_squad.json', 'w') as f:
    ...
    if st.button(f"Train +0.5 {stat} for {cost} XP"):
        if st.session_state.xp >= cost:
            for p in st.session_state.squad:
                if p['name'] == player:
                    p[stat] += 0.5
                    p['ovr'] = round(p['ovr'] + 0.1, 1)  # Simpler boost to avoid runaway stats
